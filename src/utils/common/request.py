...
class RequestUtils:
    ...
    def request_get(cls, url: str, headers = None, proxies = None, auth = None, stream = False):
        ...
        return cls.session.get(cls.get_protocol(url), headers = headers, proxies = proxies, auth = auth, stream = stream)
    
    @classmethod
    def request_post(cls, url: str, headers = None, proxies = None, auth = None, params = None, json = None):
        headers, proxies, auth = cls.get_params(headers, proxies, auth)
        
        return cls.session.post(cls.get_protocol(url), headers = headers, params = params, json = json, proxies = proxies, auth = auth)

    @classmethod
    def request_head(cls, url: str, headers = None, proxies = None, auth = None):
        headers, proxies, auth = cls.get_params(headers, proxies, auth)
        
        return cls.session.head(RequestUtils.get_protocol(url), headers = headers, proxies = proxies, auth = auth)
