...
class CookieUtils:
    ...
    def init_cookie_params(cls):
        ...
        if cls.params_invalid(user_params) and Config.User.login:
            cls.reset_user_params()

        cls.check_bili_ticket_expires()
        cls.check_login_expires()

        cls.gen_b_lsid()
        cls.get_wbi_keys()

    @classmethod
    def check_bili_ticket_expires(cls):
    ...
    def check_login_expires(cls):
        if cls.check_timestamp_expires(Config.User.login_expires) and Config.User.login:
            cls.reset_user_params()
    ...
    def generate_cookie_params(cls):
        ...
        task.append(thread_pool.submit(cls.get_buvid3_buvid4))
        task.append(thread_pool.submit(cls.get_bili_ticket))

        wait(task)

        cls.exclimbwuzhi(Config.Auth.uuid)
    ...
    def gen_b_lsid(cls):
        ...
        ret = f"{ret}_{hex(cls.get_timestamp())[2:].upper()}"
    ...
    def exclimbwuzhi(df35: str):
        ...
        RequestUtils.request_post(url, json = payload)
