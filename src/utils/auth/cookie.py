...
class CookieUtils:
    ...
    def init_cookie_params():
        def generate_cookie_params():
            ...
            CookieUtils.exclimbwuzhi(Config.Auth.uuid)
        ...
        def check_bili_ticket():
            if not Config.Auth.bili_ticket_expires or CookieUtils.check_time_diff(Config.Auth.bili_ticket_expires):
        ...
        _cookie_params = [Config.Auth.buvid3, Config.Auth.buvid4, Config.Auth.buvid_fp, Config.Auth.b_nut, Config.Auth.bili_ticket, Config.Auth.uuid]
        if is_valid(_cookie_params):
            generate_cookie_params()
    ...
    def get_bili_ticket():
        ...
        Config.Auth.bili_ticket_expires = int(datetime.now().timestamp())
    ...
    def check_time_diff(timestamp: int):
