...
def test_yt_dlp_installation():
    ...
    try:
        import yt_dlp
        print(f"[PASS] yt-dlp 版本: {yt_dlp.version.__version__}")
        return True
    except ImportError as e:
        print(f"[FAIL] yt-dlp 导入失败: {e}")
        return False
        ...
        for config in configs:
            try:
                with yt_dlp.YoutubeDL(config['opts']) as ydl:
                    print(f"[PASS] {config['name']} 测试成功")
            except Exception as e:
                print(f"[FAIL] {config['name']} 测试失败: {e}")
        
        return True
        
    except Exception as e:
        print(f"[FAIL] 配置选项测试失败: {e}")
        return False
...
def test_file_creation():
    ...
    files_to_check = [
        'src/util/download/downloader/yt_dlp_downloader.py',
...
def test_requirements_update():
    ...
    try:
        ...
        if 'yt-dlp' in requirements_content:
            print("[PASS] requirements.txt 已更新")
        else:
            print("[FAIL] requirements.txt 未更新")
            return False
        ...
        if 'yt-dlp' in toml_content:
            print("[PASS] pyproject.toml 已更新")
        else:
            print("[FAIL] pyproject.toml 未更新")
            return False
        
        return True
        
    except Exception as e:
        print(f"[FAIL] 依赖更新测试失败: {e}")
        return False
...
def main():
    ...
    if total_passed == total_tests:
        print("[SUCCESS] 所有核心测试通过! yt-dlp 下载器可以正常使用。")
        print("\n下一步:")
