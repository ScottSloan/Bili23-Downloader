...
class Notification:
    def show_dialog(parent, code: int):
        if code == 200:
            wx.MessageDialog(parent, "检查更新失败\n\n当前无法检查更新，请稍候再试", Config.APPNAME, wx.ICON_WARNING).ShowModal()

        if code == 201:
            wx.MessageDialog(parent, "当前没有可用更新", Config.APPNAME, wx.ICON_INFORMATION).ShowModal()
        
        if code == 202:
            wx.MessageDialog(parent, "未指定播放器路径\n\n尚未指定播放器路径，请添加路径后再试", Config.APPNAME, wx.ICON_WARNING).ShowModal()
